...
summaryHTML <- file.path("report","diffSummary.html")


showSummary <- 0
if (!is.null(summaryHTML)) {showSummary <- 1}
...
conditionalHrefMenuItem <- function(label,tabName=NULL,href=NULL,iconName,condition) {
	t <- menuItem(
		label,
		href = href,
		tabName = tabName,
